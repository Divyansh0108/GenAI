...
prompt = ChatPromptTemplate.from_template(
    """
    Answer the question based on provided context only. Don't include any external information. 
    Provide the most accurate and nearest possible response to the asked question.
    <context>
    {context}
    </context>
    Question: {input}
...
user_prompt = st.text_input("Ask a question from the research papers:", key="question")
...
if user_prompt:
    if "vectors" not in st.session_state:
        st.error(
            "Please create document embeddings first by clicking the 'Document Embedding' button."
        )
    else:
